...
export default function StatusButton({ gameData }) {
  const { gameId, name, background_image: backgroundImage, genres } = gameData;
  const intId = parseInt(gameId, 10);
  ...
  function toggleStatus(list, setList) {
    if (typeof list.find((game) => game.id === intId) === 'object') {
      ...
      setList((previousList) => [...previousList, { id: intId, name, background_image: backgroundImage, genres }]);
      setShowCheck(true);
    }
  }
  ...
  return (
    <div className="game-status-btn">
      ...
      <button type="button" data-testid="active-status" onClick={() => handleActiveBtnClick(activeBtn)}>
        {activeBtn}
      </button>
      <div className="dropdown-icon" ref={dropdownRef}>
        <motion.div className="dropdown-icon">
          ...
          <motion.div
            ...
            animate={isOpen ? 'enter' : 'exit'}
          >
            <motion.button variants={statusAnimate} whileHover="onHover" onClick={handleStatus}>
              Want to Play
            </motion.button>
            <motion.button variants={statusAnimate} whileHover="onHover" onClick={handleStatus}>
              Playing
